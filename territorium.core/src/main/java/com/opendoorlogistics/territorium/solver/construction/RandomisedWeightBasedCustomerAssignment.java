package com.opendoorlogistics.territorium.solver.construction;
...
import com.opendoorlogistics.territorium.solver.SolverUtils;
import com.opendoorlogistics.territorium.solver.data.CustomerClusterCostMatrix;
...
public class RandomisedWeightBasedCustomerAssignment {
	...
	private void updateQuantityViolation(ImmutableData immutable, SolutionState state) {
		...
		for (int i = 0; i < state.quantityViolations.length; i++) {
			state.quantityViolations[i] = SolverUtils.getAbsQuantityViolation(immutable.clusters.get(i),
					state.quantities[i]);
