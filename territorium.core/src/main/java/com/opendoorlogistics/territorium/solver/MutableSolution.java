...
final public class MutableSolution implements ImmutableSolution {
	...
	private class ClusterRecord {
		...
		boolean isImmutableCentre() {
			return cluster.getFixedLocation() != null;
		}
		...
		private double getQuantityViolationChangeForQuantityChange(double quantityChange){
			...
			double newViolation = SolverUtils.getAbsQuantityViolation(cluster, newQuant);
		...
		private void updateCentreAndCost() {
			...
			if (centre != null && cluster.getRefLocation() != null) {
				DistanceTime dt = problem.getTravelMatrix().get(cluster.getRefLocation().getIndex(), centre.getIndex());
				double refCost = cluster.getRefLocationCostPerUnitDistance() * dt.getDistance()
						+ cluster.getRefLocationCostPerUnitTime() * dt.getTime();
				cost.setTravel(cost.getTravel() + refCost);
			}

			// Calculate capacity violation
			cost.setQuantityViolation(SolverUtils.getAbsQuantityViolation(cluster, quantity));
		...
		private double getFixedLocationToCustomerTravelCost(CustomerRecord customer) {
			return cluster.getFixedLocation() != null
					? problem.getTravelCost(cluster.getFixedLocation(), customer.customer) : 0;
		}
